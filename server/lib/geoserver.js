...
# GeoServer processor
*/
exports.configure = function(params, target) {
        ...
        for (var ii = 0; ii < features.length; ii++) {
            ...
            switch (feature.geometry.type) {
                case 'Point': {
                    ...
                    result.geom = feature.geometry.coordinates[1] + ' ' + 
    ...
    */
    target.bbox = function(callback, queryParams, req, res) {
        return {
            result: 'blah'
        };
    };
    ...
    ### cql(queryParams)
    */
    target.cql = function(callback, queryParams, req, res) {
        getActiveServer(function(url) {
            ...
            request({ uri: targetUri }, function(err, response, body) {
                if (! err) {
                    protector.run(callback, function() {
                        return standardize(body);
                    });
                }
                else {
                    protector.report(callback, 'Invalid response from geoserver: ' + err);
                }
            });
        });
    }; // cql
